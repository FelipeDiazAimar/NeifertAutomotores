...
describe('fotos.service.subir', () => {
  it('presign → PUT → insert con la URL pública y el orden siguiente', async () => {
    ...
    expect(global.fetch).toHaveBeenNthCalledWith(1, '/api/r2/presign', expect.objectContaining({ method: 'POST' }))
    expect(global.fetch).toHaveBeenNthCalledWith(2, 'https://r2/put', expect.objectContaining({ method: 'PUT' }))
