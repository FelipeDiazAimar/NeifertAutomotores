...
import { alertaSchema } from '@/crm/lib/alertaSchema'
import { useAlertaMutations } from '@/crm/hooks/useAlertas'
...
export default function AlertaFormModal({ open, onClose, alerta, clienteFijo, onGuardado }) {
  useLockScroll(open, onClose)
  const { crear, actualizar } = useAlertaMutations()
  ...
  const {
    ...
    defaultValues: {
      titulo: '', descripcion: '', fecha: hoy(), hora: '09:00',
      asignado_a: miId ?? '', cliente_id: clienteFijo ?? '', vehiculo_id: '',
      ...alerta,
    },
  })

  // Reset cuando cambia la alerta o se abre para alta.
  useEffect(() => {
    if (!open) return
    reset({
      titulo: '', descripcion: '', fecha: hoy(), hora: '09:00',
  ...
  }, [open, alerta, clienteFijo, miId, reset])
        ...
        >
          ...
          <div className="relative flex min-h-full items-start justify-center p-4 sm:p-6">
            <motion.div
              role="dialog"
              aria-modal="true"
              aria-label={alerta?.id ? 'Editar alerta' : 'Nueva alerta'}
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                <h2 className="font-display text-lg font-bold text-ink">
                  {alerta?.id ? 'Editar alerta' : 'Nueva alerta'}
                </h2>
