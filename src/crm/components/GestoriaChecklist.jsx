        ...
        {GESTORIA_ITEMS.map(({ key, label }) => {
          ...
          return (
            <li key={key} className="flex flex-wrap items-center gap-3 p-3">
              ...
              <span className="min-w-40 flex-1 text-sm font-medium text-ink">{label}</span>
              {hecho && <span className="text-xs text-ink-3">{g?.[`${key}_fecha`]}</span>}
              <input
                defaultValue={g?.[`${key}_nota`] ?? ''}
                placeholder="Nota…"
                onBlur={(e) => guardarNota(key, e.target.value)}
                className="glass field-glass h-8 flex-1 rounded-xl px-2.5 text-sm text-ink outline-none"
              />
            </li>
          )
        })}
