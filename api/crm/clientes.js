import { fetchCrmClientes } from '../../src/server/crmCore.js'

/** Vercel Serverless Function — equivalente en producción de
 *  src/plugins/crmProxy.js (que solo corre en `vite dev`).
 *
 * GET /api/crm/clientes → cartera completa de clientes/leads del CRM viejo
 * (panel interno, login de empleado — trae `canal`, a diferencia de la API
 * pública que solo devuelve los leads que nosotros mismos empujamos).
 */
export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*')

  const syncUser = process.env.CRM_SYNC_USER
  const syncPass = process.env.CRM_SYNC_PASS
  if (!syncUser || !syncPass) {
    return res.status(501).json({
      ok: false,
      error: 'Falta CRM_SYNC_USER/CRM_SYNC_PASS en el servidor para sincronizar clientes.',
    })
  }

  try {
    const data = await fetchCrmClientes({ syncUser, syncPass })
    res.status(200).json({ ok: true, data })
  } catch (e) {
    console.error('[crm-proxy] clientes:', e.message)
    res.status(502).json({ ok: false, error: e.message })
  }
}
